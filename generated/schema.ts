...
export class VoteRemovedOther extends Entity {
  constructor(id: Bytes) {
    super();
    this.set("id", Value.fromBytes(id));
  }
  ...
  save(): void {
    let id = this.get("id");
    assert(id != null, "Cannot save VoteRemovedOther entity without an ID");
    if (id) {
      assert(
        id.kind == ValueKind.BYTES,
        `Entities of type VoteRemovedOther must have an ID of type Bytes but the id '${id.displayData()}' is of type ${id.displayKind()}`
      );
      store.set("VoteRemovedOther", id.toBytes().toHexString(), this);
    }
  }

  static load(id: Bytes): VoteRemovedOther | null {
    return changetype<VoteRemovedOther | null>(
      store.get("VoteRemovedOther", id.toHexString())
    );
  }
